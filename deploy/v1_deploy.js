...
// deployment utils (gas usage)
const {
	update_total_gas_cost,
} = require("../scripts/deployment_gas_utils");
...
module.exports = async function({deployments, getChainId, getNamedAccounts, getUnnamedAccounts}) {
	...
	// save all the deployment receipts here
	const receipts = [];

	// deploy Land ERC721 implementation v1 if required
	receipts.push(await deployments.deploy("LandERC721_v1", {
	...
	receipts.push(await deployments.deploy("LandERC721_Proxy", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
	...
	receipts.push(await deployments.deploy("LandDescriptor", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
	...
	// check if LandDescriptor needs to be set
	const land_descriptor_address =  await land_nft_proxy_contract.methods.landDescriptor().call();
	if(land_descriptor_address !== land_descriptor_deployment_address) {
	...
	if(chainId > 1) {
		...
		if(!sIlv_address) {
			receipts.push(await deployments.deploy("sILV_Mock", {
				from: A0,
				contract: "ERC20Mock",
			...
			if(!features.eq(sIlv_features)) {
				...
				console.log("sILV_Mock.updateFeatures(%o): %o", sIlv_features, receipt.transactionHash);
				receipts.push(receipt);
			}
		}
		// deploy Chainlink Aggregator Mock (if required)
		if(!chainlink_aggregator) {
			receipts.push(await deployments.deploy("ChainlinkAggregator_Mock", {
				from: A0,
			...
			({address: chainlink_aggregator} = await deployments.get("ChainlinkAggregator_Mock"));
		}
	}
	...
	receipts.push(await deployments.deploy("LandSalePriceOracle_v1", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
	...
	receipts.push(await deployments.deploy("LandSalePriceOracle_Proxy", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
		contract: "ERC1967Proxy",
	...
	receipts.push(await deployments.deploy("LandSale_v1", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
		contract: "LandSale",
	...
	receipts.push(await deployments.deploy("LandSale_Proxy", {
		// address (or private key) that will perform the transaction.
		// you can use `getNamedAccounts` to retrieve the address you want by name.
		from: A0,
		contract: "ERC1967Proxy",
	...
	// update gas usage data "gas_costs.json"
	update_total_gas_cost("v1_deploy", ...receipts)
};
