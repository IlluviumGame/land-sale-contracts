// deploy: npx hardhat deploy --network rinkeby --tags v1.0.1_upgrade
...
module.exports = async function({deployments, getChainId, getNamedAccounts, getUnnamedAccounts}) {
	...
	// deploy Land ERC721 implementation v1.0.1 if required
	await deployments.deploy("LandERC721_v1.0.1", {
	...
	// get Land ERC721 implementation v1.0.1 deployment details
	const land_nft_v101_deployment = await deployments.get("LandERC721_v1.0.1");
	const land_nft_v101_contract = new web3.eth.Contract(land_nft_v101_deployment.abi, land_nft_v101_deployment.address);
	...
	await print_nft_acl_details(A0, land_nft_v101_deployment.abi, land_nft_proxy_deployment.address);

	// prepare the upgradeTo call bytes
	const land_nft_proxy_upgrade_data = land_nft_v101_contract.methods.upgradeTo(land_nft_v101_deployment.address).encodeABI();
	...
	const land_nft_receipt = await deployments.rawTx({
		...
		data: land_nft_proxy_upgrade_data, // upgradeTo(land_nft_v101_deployment.address)
	});
	console.log("LandERC721_Proxy.upgradeTo(%o): %o", land_nft_v101_deployment.address, land_nft_receipt.transactionHash);
};
...
module.exports.tags = ["v1.0.1_upgrade", "upgrade", "v1.0.1"];
